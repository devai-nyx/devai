...
import { senseTestRobustnessCoverage } from '@devai-nyx/sensors';
...
export const senseTestRobustnessCoverageCmd = defineCommand({
  name: 'sense test-robustness-coverage',
  description:
    'Count error-path test matches; emit a test_robustness_coverage SensorReading (F3×T8)',
  authority: 'sensor',
  register(cli: CAC): void {
    cli
      .command(
        'sense-test-robustness-coverage',
        'Emit a test_robustness_coverage SensorReading (F3×T8)',
      )
