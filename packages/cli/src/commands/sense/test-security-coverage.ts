...
import { senseTestSecurityCoverage } from '@devai-nyx/sensors';
...
export const senseTestSecurityCoverageCmd = defineCommand({
  name: 'sense test-security-coverage',
  description:
    'Count security-pattern test matches; emit a test_security_coverage SensorReading (F3×T6)',
  authority: 'sensor',
  register(cli: CAC): void {
    cli
      .command(
        'sense-test-security-coverage',
        'Emit a test_security_coverage SensorReading (F3×T6)',
      )
