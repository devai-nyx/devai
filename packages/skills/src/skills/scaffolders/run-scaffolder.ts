/**
 * Phase 18.F: shared run-helper for every SKILL-scaffold-*.
 *
 * Each scaffolder differs in (1) which template ids it consumes
 * (per the matched stack-adapter pack's `templates` registry) and
...
// ---------------------------------------------------------------------
// Local SkillResult shape (decoupled from skills/index.ts to avoid
// a circular runtime import; the structural shape matches SkillResult
// exactly).
// ---------------------------------------------------------------------

export interface ScaffolderSkillResult {
...
export interface ScaffolderSpec {
   ...
   * Returning [] is legal — means "this scaffolder has nothing to do
   * for this blueprint" (e.g. SKILL-scaffold-ui called against a
   * blueprint with no api.resources).
   */
...
export function runScaffolder(opts: RunScaffolderOptions): ScaffolderSkillResult {
  const { spec, ctx } = opts;
  ...
  const resolution = resolveStackAdapterPack({
    repoRoot: packsRoot,
    adopterRoot: ctx.repoRoot,
  });
  const pack = resolution.matched;
  if (pack === null) {
    return {
      skill_id: spec.skillId,
      status: 'skipped',
  ...
  for (const id of spec.templateIds) {
    ...
    if (entry.consumed_by !== spec.skillId) continue;
  ...
  if (consumedTemplates.length === 0) {
    return {
      ...
      status: 'skipped',
      notes: [
        `Pack '${pack.id}' declares no templates consumed by '${spec.skillId}'. Expected ids: ${spec.templateIds.join(', ')}.`,
      ],
      evidence: { blueprint_id: blueprint.id, stack_adapter_pack: pack.id },
    };
  }
  ...
  if (tasks.length === 0) {
    return {
      skill_id: spec.skillId,
      status: 'skipped',
      notes: [`Blueprint '${blueprint.id}' produced no scaffold tasks for '${spec.skillId}'.`],
      evidence: { blueprint_id: blueprint.id, stack_adapter_pack: pack.id },
    };
  }
