// The explicit schema roster (W02.c: explicitness is a feature; laziness is the mechanic).
// 86 contract schemas + 3 infrastructure (common-defs, record-meta, meta).
// Guarded: the roster-dir bijection is a T2 contract; cardinality derives from the directory.
export const ROSTER = [
  'action-registry.schema.json',
  'action-result.schema.json',
  'active-control-census.schema.json',
  'actions-evidence-gate-authorization.schema.json',
  'actions-list-output.schema.json',
  'adr.schema.json',
  'affected-test-graph.schema.json',
  'affected-test-execution.schema.json',
  'agent-routing-policies.schema.json',
  'agent-run.schema.json',
  'api-map.schema.json',
  'assessment.schema.json',
  'authority-policy.schema.json',
  'authority-session.schema.json',
  'candidate-manifest.schema.json',
  'check-suites.schema.json',
  'ci-optimisation-evidence.schema.json',
  'claim-runtime-inputs.schema.json',
  'common-defs.schema.json',
  'commit-validation-plan.schema.json',
  'control-provenance.schema.json',
  'coverage-matrix.schema.json',
  'current-claims.schema.json',
  'data-model-inventory.schema.json',
  'dep-graph.schema.json',
  'documentation-information-architecture.schema.json',
  'error.schema.json',
  'evidence.schema.json',
  'forbidden-actions.schema.json',
  'genesis-attestation.schema.json',
  'glob-guards.schema.json',
  'glossary-entry.schema.json',
  'github-actions-features.schema.json',
  'inv-candidate.schema.json',
  'inv-override.schema.json',
  'invariant.schema.json',
  'inventory.schema.json',
  'journey.schema.json',
  'local-evidence-manifest.schema.json',
  'meta.schema.json',
  'model-runtime-registry.schema.json',
  'module-blueprint.schema.json',
  'mutation-intent.schema.json',
  'phase-closure.schema.json',
  'population-registry.schema.json',
  'prior-finding-registry.schema.json',
  'project-config.schema.json',
  'prompt-composition.schema.json',
  'proof-epoch.schema.json',
  'rbac-inventory.schema.json',
  'record-meta.schema.json',
  'release-control.schema.json',
  'remediation-closure-matrix.schema.json',
  'repo-introspection.schema.json',
  'review-obligations.schema.json',
  'review-result.schema.json',
  'review-repair-evidence.schema.json',
  'review-scope-manifest.schema.json',
  'review-state.schema.json',
  'review-transport.schema.json',
  'reviewer-binding.schema.json',
  'rgr.schema.json',
  'round-close-controls.schema.json',
  'round-close-manifest.schema.json',
  'round-close-profile.schema.json',
  'round-convergence.schema.json',
  'round-declaration.schema.json',
  'round-execution.schema.json',
  'routes-inventory.schema.json',
  'rtd-manifest.schema.json',
  'runtime-charter.schema.json',
  'scorecard-na-config.schema.json',
  'scorecard.schema.json',
  'sense-presets.schema.json',
  'sensor-input-spec.schema.json',
  'sensor-registry.schema.json',
  'sensor-reading.schema.json',
  'skill-manifest.schema.json',
  'stack-adapter.schema.json',
  'subprocess-effects.schema.json',
  'task-execution-evidence.schema.json',
  'task-freshness.schema.json',
  'task.schema.json',
  'test-weakening-config.schema.json',
  'trace.schema.json',
  'translation-witness.schema.json',
  'triage.schema.json',
  'use-cases.schema.json',
  'validation-result.schema.json',
] as const;
export type SchemaName = (typeof ROSTER)[number];
export const INFRASTRUCTURE: readonly SchemaName[] = [
  'common-defs.schema.json',
  'record-meta.schema.json',
  'meta.schema.json',
];
