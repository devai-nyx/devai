...
The pre-R-0006 governance-alignment campaign stops after its control-only merge and
exact-main CI. During that campaign do not append the R-0006 declaration decision, run
E1 through E5, implement workspace control machinery, or begin B0 through B9.

At a separately authorized and live-revalidated R-0006 launch, execute E0 through E5
first. E0 binds the exact amended contract and SHA-256 digests of this prompt and the
plan before any substantive work. E1 proves the control defects red; E2 defines their
semantics; E3 changes workspace tooling only; E4 verifies acceptance and adversaries;
E5 audits and independently reviews the exact prelude candidate. Any finding returns to
the owning role, requires a complete same-class sweep, reconvergence, and a fresh E5
review. Do not retain a PASS across a repair.

B0 through B9 remain blocked until E5 PASS. Then freeze the starting denominator and
config in the round audit. The Architect makes two separate BL-035 decisions. The
Inspector writes behavior-first reds and coverage tests; the Engineer fixes the plant
and measurement plumbing. Never exclude a valid source merely to raise a percentage.

Before the final review, record `implementation_subject` and converge twice with the
second pass no-write/clean; prove every governed identity in a candidate-only clone; and
rehearse source ancestry plus a closure-only PC branch in isolation. Freeze the exact
`review_candidate`. After PASS, permit only the exact Auditor review record and solely
