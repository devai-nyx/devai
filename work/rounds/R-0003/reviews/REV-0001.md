---
id: REV-0001
title: Constitution 1.0.0 — consolidated content draft
type: constitution-draft
status: draft
date: 2026-07-23
authority: Architect (ratification = BR-1/W01)
supersedes: null
superseded_by: null
provenance: devai-original CONSTITUTION.md@0.6.0 (live articles, mechanical transposition) + dossier Part V (Article 41) + manifest PROMOTE set
---
...
- F1 working-papers paths: `docs/work/` — round plans, prompts, and round inventories under Architect authority, except `docs/work/*/audit/`, which is the designated Auditor observation path (Article 7). Observation artifacts written there carry no authority over the reference signal.
...
- F5 paths: `.devai/` (excluding `inventory/`, `worktrees/`, and `state/`). Configuration and harness machinery are modified only via `devai upgrade`, never by ordinary disciplines.
- F5 state paths: `.devai/state/`. Runtime-recorded ledger and observation state (evidence chain, sensor readings, closures, counters), written by DEVAI verbs as a side effect of their execution. Such writes are attributed to the executing verb, not to the session role; the session that produced the records commits them regardless of its declared role. No human or agent edits state files by hand.
...
## W01 ANNEX — deltas to work into article text before 1.0.0 ratification (review checklist, not article text)

The forty articles above are the live 0.6.0 text (all six predecessor amendments already integrated). Beyond adding Article 41, W01 must decide these deltas:

1. **Article 6 rewrite** — path enumeration replaced by the Part VII static-prefix table: law/ (glossary joint), product/, work/rounds/, work/audit/, docs/, record/ (machine-only), .devai/pin+config (upgrade-only), .devai/state (verbs), packages/+workspace config, tests/ + packages/*/tests/, scratch/, root prose files, host-tool config dirs (contents classified per class). Fixes in passing: the F4 enumeration drift (.devai/inventory vs state/inventory → record/derived/).
2. **Change-control clause (ex-D-136, sole-carried)** — add to Article 9 or 40: weakening any fail-closed authority property requires a new explicit decision; changes touching Articles 6–10 require constitutional amendment.
3. **Zero-exemption mutator gate (ex-D-136, sole-carried)** — constitutional or ADR-003-successor placement: the mutator call-site denominator is mechanically derived; zero unauthorized sites, zero exemptions; the gate must demonstrably fail on stale fixtures.
4. **Universal uncheckable-evidence rule (ex-D-168)** — Article 32/33 territory: judge-only *or otherwise independently uncheckable* evidence never establishes readiness (currently only the semantic-review instance is stated in ADR-006).
5. **Promotion review of the 16 PROMOTE records** — most are already embodied (D-1→Art 2; D-2→Arts 4–5; D-3→Arts 6–7; D-6→Art 11; D-7→Art 12; D-10→Arts 25–27; D-24→Arts 32–33; D-30→Art 36; D-33→Art 38; D-126→0.4.0 text; D-156→Art 11/INV-020 via 0.6.0). W01 verifies each and absorbs residue: D-4 (embedded-package axiom — not yet articled), D-12 (coupled-triplet doctrine — Art 24 check), D-51/D-57 (framework-not-product scope statement — Art 1 candidate), D-146/D-164 (evidence-promotion contract — likely stays ADR-tier).
6. **Numbering/altitude sweep** — no operational values in article text (0.3.0 doctrine); anchor-slug stability check against law/ tree names.
