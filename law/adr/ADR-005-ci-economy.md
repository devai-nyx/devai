...
session in BR-1/W04. Source texts: ADR-CI-ECONOMY.md.
...
**Scope (from the disposition map):** Hard/advisory rule split, the reusable evidence gate (never silently opens), gate-staged profile conditioning (downgrades only rule 4), local-evidence generalization (manifest schema, collect/verify verbs, forbidden-paths floor never shrinks, PR events never evidence_mode). Ex-D-115/116/117.
