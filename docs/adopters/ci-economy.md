...
- **(a) per-package ephemeral databases**: keep the single CI Postgres _service_, but create one database per package from a migrated template, inject it per suite, drop it after; or
