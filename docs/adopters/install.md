...
Run `devai doctor --format human` to surface which bridges are available on your machine. The `llm-bridges` row reports `claude-cli` and `codex-cli` presence, version, and usability with an actionable hint per family.
