...
describe('devai adopt hooks install (D-123, item 5)', () => {
  skipIfNotBuilt('plan-only mode reports the target without writing anything', () => {
  ...
  skipIfNotBuilt('prefers .husky/<hook> over .git/hooks when a .husky dir is present', () => {
  ...
  skipIfNotBuilt('accepts a non-mutating post-merge installation plan', () => {
  ...
  skipIfNotBuilt('allows only Architect authority to install the post-merge adapter', () => {
  ...
  skipIfNotBuilt('--format human output names the manager, hook, and command', () => {
