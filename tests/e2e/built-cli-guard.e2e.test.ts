 ...
 * a skipIfNotBuilt pattern: when packages/cli/dist/bin.js is absent
...
describe('built-CLI guard', () => {
  it('packages/cli/dist/bin.js exists — skipIfNotBuilt suites are actually running', () => {
    expect(
      existsSync(BIN),
      'dist/bin.js missing: every skipIfNotBuilt suite is silently skipping. Build with `tsc -b` before running integration tests.',
    ).toBe(true);
  });
});
